...
func TestAPI_ACLToken_List(t *testing.T) {
	...
	// ensure the 5th token is the root main token
...
func TestAPI_ACLToken_Clone(t *testing.T) {
	...
	main, _, err := acl.TokenReadSelf(nil)
	require.NoError(t, err)
	require.NotNil(t, main)

	cloned, _, err := acl.TokenClone(main.AccessorID, "cloned", nil)
	require.NoError(t, err)
	require.NotNil(t, cloned)
	require.NotEqual(t, main.AccessorID, cloned.AccessorID)
	require.NotEqual(t, main.SecretID, cloned.SecretID)
	require.Equal(t, "cloned", cloned.Description)
	require.ElementsMatch(t, main.Policies, cloned.Policies)
